...
if __name__=="__main__":
  os.chdir('/usr/local/verify_code/')
  '''
  work_dir = './train_imgs/'
  ...
  if os.path.exists(pickle_file):
    ...
    pickle.dump({'X':X, 'Y':y},f,0)  
    f.close()  
  predict_img = sys.argv[1]
  m = hashlib.md5()
  m.update(predict_img.encode('utf8'))
  tmp_dir = 'tmp/'+time.strftime('%Y-%m-%d',time.localtime(time.time()))+'/'+m.hexdigest()
  if(not os.path.exists(tmp_dir)):
    os.makedirs(tmp_dir)
  img_split('code.jpg', predict_img, '9999', tmp_dir)
  #spilt_imgs_dir = './'+tmp_dir
  prdict_code = ''
  for i in range(4):
    prdict_code = prdict_code+classify(img2vector(tmp_dir+"/9_"+str(i)+'_code.jpg.txt'), array(X), y, 1)
  print(prdict_code)
